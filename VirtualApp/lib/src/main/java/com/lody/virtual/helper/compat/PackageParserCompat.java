...
public class PackageParserCompat {
    ...
    public static PackageParser createParser(File packageFile) {
        if (API_LEVEL >= 28) {
            return PackageParserPie.ctor.newInstance();
        } else if (API_LEVEL >= M) {
    ...
    public static Package parsePackage(PackageParser parser, File packageFile, int flags) throws Throwable {
        if (API_LEVEL >= 28) {
            return PackageParserPie.parsePackage.callWithException(parser, packageFile, flags);
        } else if (API_LEVEL >= M) {
    ...
    public static ServiceInfo generateServiceInfo(Service service, int flags) {
        if (API_LEVEL >= 28) {
            return PackageParserPie.generateServiceInfo.call(service, flags, sUserState, myUserId);
        } else if (API_LEVEL >= M) {
    ...
    public static ApplicationInfo generateApplicationInfo(Package p, int flags) {
        if (API_LEVEL >= 28) {
            return PackageParserPie.generateApplicationInfo.call(p, flags, sUserState);
        } else if (API_LEVEL >= M) {
    ...
    public static ActivityInfo generateActivityInfo(Activity activity, int flags) {
        if (API_LEVEL >= 28) {
            return PackageParserPie.generateActivityInfo.call(activity, flags, sUserState, myUserId);
        } else if (API_LEVEL >= M) {
    ...
    public static ProviderInfo generateProviderInfo(Provider provider, int flags) {
        if (API_LEVEL >= 28) {
            return PackageParserPie.generateProviderInfo.call(provider, flags, sUserState, myUserId);
        } else if (API_LEVEL >= M) {
    ...
    public static PackageInfo generatePackageInfo(Package p, int flags, long firstInstallTime, long lastUpdateTime) {
        if (API_LEVEL >= 28) {
            return PackageParserPie.generatePackageInfo.call(p, GIDS, flags, firstInstallTime, lastUpdateTime,
                    null, sUserState);
        } else if (API_LEVEL >= M) {
    ...
    public static void collectCertificates(PackageParser parser, Package p, int flags, boolean skipVerify) throws Throwable {
        if (API_LEVEL >= 28) {
            PackageParserPie.collectCertificates.callWithException(parser, p, skipVerify);
        } else if (API_LEVEL >= N) {
